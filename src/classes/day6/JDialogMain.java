...
public class JDialogMain extends JDialog {
    ...
    //부모창에서 사용자의 요청에 따라 화면이 켜지고 타이틀도 바뀌고 setVisible바뀌고
    //또 setSize바뀌고 JTextField도 상세보기일때는 읽기만 (비활성화)
    //입력과 수정일 때는 활성화 시켜주어야 한다.
    //아래 메소드는 JFrameMain에서 호출한다.
    ...
    public void setEditable(boolean editable){
        jtf_name.setEditable(editable);//true입력,수정, false 비활성화-only read
    }
    public void initDisplay(){
        jp_center.setLayout(null);//FlowLayout이 디폴트인데 좌표값으로 배치함
