...
Class객체를 사용합니다.
1. instanceof
객체가 특정 클래스나 인터페이스의 인스턴스인지 확인할 수 있습니다.
2. Class객체를 통한 타입 확인
Java에서는 모든 클래스가 런타임에 Class객체로 표현됩니다.
...
주의 : 서버 성능을 저하시키므로 제한적으로 사용할 것.

RTTI의 단점 : 성능저하, 디자인 의존성 증가시켜 유지보수 저하를 가져옴
필요할 때만 사용합니다.

Spring Framework를 모방하기
가능한 이유는 자바에서 RTTI와 리플렉션을 지원하기 때문에 가능함.
- RTTI는 기본적으로 자바가 제공하는 컴파일된 클래스 정보를 런타임에 활용하는 방식
- 컴파일시에 알려진 클래스만 처리할 수 있음.
- 리플렉션을 사용하면 런타임에 동적으로 클래스의 구조(필드, 메소드)를 탐색하거나 수정할 수 있음.
- 프레임워크/라이브러리에서 객체 타입을 기반으로 동적 처리 가능함
